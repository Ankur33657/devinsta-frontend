...
import { Heart, XCircle, Twitter, Github, Compass } from "lucide-react";
...
const Mainpage = () => {
  ...
  return (
    <>
      <main className="flex-grow h-full flex items-center justify-center p-4 sm:p-8 relative overflow-x-hidden">
        {/* Background Glows */}
        ...
        {!data[idx] && (
          <div className="flex flex-col items-center justify-center text-center glass-dark border border-white/10 rounded-3xl p-12 max-w-md mx-auto shadow-xl">
            ...
            <p className="text-slate-400 text-sm mb-6 max-w-xs">
              Looks like there are no new developers to connect with right now.
              Check back later or explore more profiles.
            </p>

            <button
              onClick={() => window.location.reload()}
              className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-emerald-500 text-white rounded-xl font-semibold hover:scale-105 transition"
            >
              Refresh
            </button>
          </div>
            ...
            <div className="glass-dark rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10 hover:border-white/20 transition-all hover:shadow-emerald-500/5 hover:-translate-y-1 flex flex-col md:flex-row">
              <div className="relative aspect-[4/5] md:aspect-auto md:w-1/2 overflow-hidden group">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  ...
                  <span className="px-3 py-1 bg-emerald-500 backdrop-blur-md rounded-full text-[10px] uppercase tracking-widest font-bold text-white border border-emerald-500/30">
                    {data[idx]?.isPremium ? "Verified" : "Available"}
                  </span>
                </div>
              </div>
              ...
              <div className="p-8 md:p-12 md:w-1/2 flex flex-col justify-center">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-3xl md:text-5xl font-bold text-white tracking-tight">
                    {data[idx]?.firstName?.slice(0, 8) || "Adventurer"}
                    {data[idx]?.firstName?.length > 8 ? "..." : " ,"}
                    {data[idx]?.dob ? Utils.getAge(data[idx]?.dob) : "?"}
                  </h2>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <button
                    className="group relative flex items-center justify-center gap-3 py-5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-2xl border border-red-500/20 hover:border-red-500/40 transition-all overflow-hidden"
                  ...
                  <button
                    className="group relative flex items-center justify-center gap-3 py-5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-2xl border border-emerald-500/20 hover:border-emerald-500/40 transition-all overflow-hidden"
